...
def on_message(wsapp, msg):
	...
	if "type" in data and data["type"] == "interrupt":
		config_type = "中断"
		global_var.get("interrupt_tasks_waiting_queue").put(
			{
				"data": data,
			}
		)
	else:
		global_var.get("tasks_config_waiting_queue").put(
			{
				"data": data,
			}
		)
...
def ws_client():
	...
	while True:
		try:
			wsapp = websocket.WebSocketApp(ws_url, on_open=on_open, on_message=on_message, on_error=on_error, on_close=on_close)
