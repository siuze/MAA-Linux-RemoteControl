...
from nonebot.params import EventMessage, RegexDict
from nonebot.permission import SUPERUSER
from nonebot.plugin import PluginMetadata
from nonebot.typing import T_State


from nonebot import Bot, get_bot, get_driver, logger, on_regex, require
...
maa = on_regex(r"^((maa)|(MAA)|(Maa))\s?启动(?P<index>([0-9, ]+)?)$", permission=SUPERUSER, priority=10, block=True)

maa_update = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?更新$", permission=SUPERUSER, priority=101, block=True)
maa_screenshot = on_regex(
	r"^/?((maa)|(MAA)|(Maa))\s?截图$",
	#   permission=SUPERUSER,
	priority=101,
	block=True,
)
maa_stop = on_regex(r"^((maa)|(MAA)|(Maa))\s?停止$", permission=SUPERUSER, priority=101, block=True)
maa_stop_config = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?停止当前(任务|配置)?$", permission=SUPERUSER, priority=101, block=True)
maa_schedule_ctrl = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?定时(任务)?(?P<bool>(打开|开启|启动|关闭|停止))$", permission=SUPERUSER, priority=101, block=True)
maa_recall_ctrl = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?(回调|通知|回调消息|通知消息|消息)(?P<bool>(打开|开启|启动|关闭|停止))$", permission=SUPERUSER, priority=101, block=True)
maa_schedule_delay = on_regex(r"^((maa)|(MAA)|(Maa))\s?(定时)?(任务)?(延迟|暂停|跳过)\s?(?P<wait>([0-9]+))$", permission=SUPERUSER, priority=101, block=True)
maa_config = on_regex(
	r"^/?((maa)|(MAA)|(Maa))\s?(修改)?配置$",
	#   permission=SUPERUSER,
	priority=101,
	block=True,
)

maa_start_user = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?启动$", priority=200, block=True)
maa_stop_user = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?(停止|关闭)$", priority=200, block=True)
maa_schedule_delay_user = on_regex(r"^/?((maa)|(MAA)|(Maa))\s?(定时)?(任务)?(延迟|暂停|跳过)\s?(?P<wait>([0-9]+))$", priority=200, block=True)
...
async def maa_schedule_delay_user_handle(
	event,
	match: Annotated[dict[str, Any], RegexDict()],
):
...
async def maa_schedule_ctrl_handle(
	match: Annotated[dict[str, Any], RegexDict()],
):
...
async def maa_recall_ctrl_handle(
	match: Annotated[dict[str, Any], RegexDict()],
):
	if match["bool"] in ["打开", "开启", "启动"]:
...
async def maa_schedule_delay_handle(
	match: Annotated[dict[str, Any], RegexDict()],
):
	...
	try:
		if msg.include("text"):
			...
			if state["op"] == "fix":
				...
				config.set_tasks(state["config"])
				await maa_config.finish("配置已修改", reply_message=True, at_sender=False)
			elif state["op"] == "del":
				if text in ["是", "确认", "确定"]:
