...
#### 一个配置的主要格式如下：    
 ...
 - **请参阅[tasks_example.json](./config_example/tasks_example/)获取更多任务配置说明**    
 - **json不能有注释，请注意删去！**  
...
```
> **`condition`参数说明：** 
> - `condition`参数的值只能为一个字典（或者叫做对象）；   
> - `condition`字典内的每个键值对将对应一个条件；   
> - `condition`字典内的每个条件之间是`and`（逻辑与）的关系，也就是说只有所有条件满足才执行该任务；   
> - `condition`字典内的每个条件参数可取如下值之一：`weekday`、`hour`、`not`、`and`、`or`，条件的编写要求见`1.`、`2.`、`3.`、`4.`、`5.`；   
> 1. `weekday`条件的值只能为一个数组，数组内填入星期序号，1到7对应星期一到星期日。只要今天对应的星期序号在数组内，该条件就成立；  
> 2. `hour`条件的值只能为一个字典，字典内允许最多存放两种时间判断关键词：`>`和`<`以及对应的小时时段值（整数类型0到23），当时间大于且小于各自对应的时段时，条件成立；  
> 3. `not`条件的值只能为一个子条件字典，字典内只能有一个键值对，子条件的关键字只能为：`id`或`not`或`and`或`or`；MAA-LRC会进一步这些子条件是否成立，检查方法与`3.`、`4.`、`5.`、`6.`中描述的一致；  
> 4. `and`条件的值只能为一个数组，数组存放着待判断的多个子条件，每个子条件是个仅存放着一个键值对的字典，字典内关键字的取值只能为 `id`或`not`或`and`或`or`。maa-lrc将逐个检查子条件（检查方法与`3.`、`4.`、`5.`、`6.`中描述的一致），只有当所有子条件都满足时，条件成立；
> 5. `or`条件的值只能为一个数组，数组存放着待判断的子条件，每个子条件是个仅存放着一个键值对的字典，字典内关键字的取值只能为 `id`或`not`或`and`或`or`。maa-lrc将逐个检查子条件（检查方法与`3.`、`4.`、`5.`、`6.`中描述的一致），只要有一个子条件都满足，条件就成立；   
> 6. 当子条件为`id`时，会检查`id`对应的`Fight`任务是否成功执行了，只有当该任务实际执行作战次数大于0（也就是既没有被跳过也没有因为理智不够而执行不了）时，条件成立；  
> 7. 通过上述的递归检查方法，任务执行条件允许嵌套判断语句实现更全面的控制，比如 “如果前面的作战任务都没被执行时才执行当前任务” 这个需求，就可以使用：
```json
"condition": {
					"and": [
						{"not": {"id": "前面的作战任务A"}},
