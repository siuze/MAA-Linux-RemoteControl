	...
	"tasks": 	//子任务序列，将依次执行这里面的任务
		[
			...
			},
			{
				...
				"params": {
					"task_names": [//填写最小任务动作
						"进入任务"	//下面这几个这是我自定义的任务动作，实现在日常周常建立界面截图的功能
					]
				}
			},
			{
...
请参考MAA官方的[集成文档](https://maa.plus/docs/%E5%8D%8F%E8%AE%AE%E6%96%87%E6%A1%A3/%E9%9B%86%E6%88%90%E6%96%87%E6%A1%A3.html)填写任务参数   
 ...
 - Award 领取日常奖励  
 - Custom 自定义任务  
请注意 进驻|简报|公招|日常|周常等界面截图任务是由一系列自定义任务下调用maa-lrc自定义的任务动作实现的，并且定义的很简陋，自适应性差，所以只能紧跟在对应的官方任务后面运行。
...
脚本会创建两个线程，一个维护全局变量wsapp，实现保持ws连接。收到消息时，会把配置加入到全局变量tasks_config队列中。  
  
另一个线程不断检查tasks_config队列，将其中的配置传给全局变量mymaa运行任务，运行完成后将配置从队列中取出删去，如此反复直到队列为空。当队列为空时将清除全局变量mymaa释放内存，以减少长期运行过程中内存泄露的可能（虽然不知道到底有没有用）。下一次有新配置时会重新构造mymaa实现激活-休眠的循环过程。  
