...
def my_callback(msg, details, arg) -> None:
	...
	if "what" in js:
		...
		if what == "UuidGot":
			...
			text = "截图失败，可能是ADB配置出现问题或Android 11无线调试变化了端口，尝试重新连接"
			lg.info(text)
			...
			my_maa.log["recruit"] += "公招无招聘许可\n"
		elif what == "RecruitTagsSelected":
			my_maa.log["recruit"] += "公招选中："
			my_maa.log["recruit"] += "【 "
...
class MAA:
	def __init__(self) -> None:
		self.log = {"connect": "", "fight": {"stages": {}, "drops": {}, "msg": "", "log": "", "sanity": ""}, "recruit": "", "infrast": ""}
	...
	def connect(self, init=False, retry=0):
		...
		lg.info(text)
		self.log["connect"] += "\n" + text
		if self.asst.connect(adb_path=self.asst_config["connection"]["adb"], address=f'{self.asst_config["connection"]["ip"]}:{self.asst_config["connection"]["port"]}', config=self.asst_config["connection"]["config"]):
			text = "连接成功"
			lg.info(text)
			self.log["connect"] += "\n" + text
	...
	def screenshot(self, file_name, rb=False):
		...
		try:
			...
			shell_cmd = f'{self.asst_config["connection"]["adb"]} -s ' f'{self.asst_config["connection"]["ip"]}:{self.asst_config["connection"]["port"]} ' f'exec-out screencap -p > ' f'{img_path}'
	...
	def interrupt_tasks_handler(self, data: dict):
		...
		recall = {"payload": f"{currentTime}\n开始运行中断配置：{config_name}", "status": "BEGIN", "type": "interrupt_config_notice", "config": self.config_content_without_tasks(data)}
		global_var.get("send_msg_waiting_queue").put(recall)
		task_index = 0
		while task_index < len(data["tasks"]):
			...
			lg.info(task)
			recall = {"devices": uuid.UUID(int=uuid.getnode()).hex[-12:].upper(), "user": "", "task": task["id"], "task_type": task["type"], "status": "SUCCESS", "payload": "", "image": "", "type": "interrupt_task_report", "name": config_name, "task_config": task, "config": self.config_content_without_tasks(data)}
			task_index += 1
		...
		currentDateAndTime = datetime.datetime.now()
		currentTime = currentDateAndTime.strftime("%Y-%m-%d %H:%M:%S")
		recall = {"payload": f"{currentTime}\n运行结束：{config_name}", "status": "END", "type": "interrupt_config_notice", "config": self.config_content_without_tasks(data)}
		global_var.get("send_msg_waiting_queue").put(recall)
	...
	def tasks_handler(self, data: dict):
		...
		currentDateAndTime = datetime.datetime.now()
		currentTime = currentDateAndTime.strftime("%Y-%m-%d %H:%M:%S")
		recall = {"payload": f"{currentTime}\n开始运行配置：{config_name}", "status": "BEGIN", "type": "normal_config_notice", "config": self.config_content_without_tasks(data)}
		global_var.get("send_msg_waiting_queue").put(recall)
		task_index = 0
		retry = 0
		success_tasks = []
		...
		while task_index < len(data["tasks"]):
			...
			if "id" not in task:
				task["id"] = f"{task['type']}_{task_index}"
			lg.info(task)
			recall = {"devices": uuid.UUID(int=uuid.getnode()).hex[-12:].upper(), "user": "", "task": task["id"], "task_type": task["type"], "status": "SUCCESS", "payload": "", "image": "", "type": "normal_task_report", "name": config_name, "task_config": task, "config": self.config_content_without_tasks(data)}
			task_index += 1
		...
		currentDateAndTime = datetime.datetime.now()
		currentTime = currentDateAndTime.strftime("%Y-%m-%d %H:%M:%S")
		recall = {"payload": f"{currentTime}\n运行结束：{config_name}", "status": "END", "type": "normal_config_notice", "config": self.config_content_without_tasks(data)}
		global_var.get("send_msg_waiting_queue").put(recall)
